...
app.post('/images/post' , upload.single('images') , function(req, res , error){
    ...
    const query = "insert into photo2 (name , photo) values (?,?)";
...
app.get('/images/get', function(req, res) {
    const query = "SELECT * FROM photo2";
...
app.put('/images/put/:id' ,upload.single('images') , function(req,res){
    ...
    const query = "UPDATE photo2 SET name = ?, photo = ? WHERE id = ?";
...
app.delete('/image/delete/:id' , function(req ,res){
    const query = "DELETE FROM PHOTO2 WHERE ID = ?";
    const id = req.params.id;
...
app.use('/uploads', express.static(path.join(__dirname, 'uploads2')));

app.listen(4016 , (error)=>{
    if(error){
        console.log(error);
    }
